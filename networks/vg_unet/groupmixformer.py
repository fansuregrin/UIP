...
class EfficientAtt(nn.Module):
    def __init__(self, dim, num_heads=8, seg=5, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.,):
        super().__init__()
        ...
        self.aggregator = Aggregator(dim=dim, seg=seg)
...
class GMA_Block(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path_rate=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, seg=5
                 ):
        super().__init__()
        self.cpe = ConvPosEnc(dim=dim, k=3)
        self.norm1 = norm_layer(dim)
        self.att = EfficientAtt(
            dim, num_heads=num_heads, seg=seg, qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop,)
