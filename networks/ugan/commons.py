...
class Gradient_Penalty(nn.Module):
    ...
    def forward(self, D, real, fake):
        ...
        eps = self.Tensor(np.random.random((real.size(0), 1, 1, 1))).to(real.device)
        # Get random interpolation between real and fake samples
        interpolates = (eps * real + ((1 - eps) * fake)).requires_grad_(True)
        d_interpolates = D(interpolates)
        fake = autograd.Variable(self.Tensor(d_interpolates.shape).fill_(1.0).to(real.device), requires_grad=False)
